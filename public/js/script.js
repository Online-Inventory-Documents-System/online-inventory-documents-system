// public/js/script.js
// Full client script, copy-paste ready. Update RENDER_API_ORIGIN to your Render URL.
...
function renderDocuments(docs) {
    ...
    docs.forEach(doc => {
        ...
        const timeStr = doc.date ? new Date(doc.date).toLocaleString() : '';
...
function renderLogs() {
    ...
    reversedLog.forEach(log => {
        ...
        item.innerHTML = `[${escapeHtml(timeStr)}] <b>${escapeHtml(log.user)}</b>: ${escapeHtml(log.action)}`;
...
function renderDashboardData(){
  ...
  if(tbody) {
    ...
    activityLog.slice().reverse().slice(0, 5).forEach(log => {
      ...
      const timeStr = log.time ? new Date(log.time).toLocaleString() : '';
      tr.innerHTML = `<td>${escapeHtml(log.user || 'Admin')}</td><td>${escapeHtml(log.action)}</td><td>${escapeHtml(timeStr)}</td>`;
...
async function confirmAndDeleteItem(id){
  const item = inventory.find(x => (x._id || x.id) === id || x.id === id);
  if(!item) return;
...
async function bindProductPage(){
  ...
  if(id){
    try{
        ...
        const it = items.find(x => String(x._id || x.id) === String(id));
        if(!it) { alert('Item not found'); return; }
        
        if(qs('#prod_id')) qs('#prod_id').value = it._id || it.id || it.id;
...
async function deleteDocumentConfirm(id) {
    const doc = documents.find(d => String(d._id || d.id) === String(id) || d.id === id);
    if(!doc) return;
